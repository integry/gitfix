...
function buildUpdatedConfig(options: BuildUpdatedConfigOptions): Record<string, unknown> {
  const { existingConfig, baseBranch, granularity, contextLevel, compress, contextRepositories, requestGenerationModel, excludedFiles } = options;
  ...
  // Preserve manualFiles and autoFiles if they exist
  if (existingConfig.manualFiles) updatedConfig.manualFiles = existingConfig.manualFiles;
  if (existingConfig.autoFiles) updatedConfig.autoFiles = existingConfig.autoFiles;
  // contextCache is intentionally NOT copied - it will be regenerated by generateContextPreview
  if (contextRepositories) updatedConfig.contextRepositories = contextRepositories;
  ...
  return async function previewContext(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      if (deps.db && hasConfigUpdates) {
        ...
        const updatedConfig = buildUpdatedConfig({ existingConfig, baseBranch, granularity, contextLevel, compress, contextRepositories, requestGenerationModel, excludedFiles });
