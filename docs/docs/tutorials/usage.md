...
- **Model labels** (`llm-...`) select the agent and model, for example `llm-claude-fable51`, `llm-codex-astra`, `llm-antigravity-flash38-high`, `llm-opencode-big-pickle`, `llm-vibe-mistral`, `llm-muse-spark13`. Adding several model labels to one issue produces one run, branch, and PR per model, so you can compare results.
- **`base-<branch>`** targets a non-default base branch, for example `base-develop`.
- **`level-<level>`** overrides the configured reasoning level for that issue, for example `level-xhigh` or `level-max`. Valid levels are `none`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`, `ultra`, `ultracode`, and `auto`. `none` and `minimal` apply to Muse Code runs only and are omitted for other runtimes; `auto`, `ultracode`, and `ultra` are omitted for Muse Code. Claude receives `auto` as adaptive effort; Codex omits the override for `auto` and uses its own default selection. Unlike model labels, adding a reasoning level does not fan out extra runs; if multiple reasoning labels are present, the highest-priority valid level applies to every run created for the issue. For PR follow-ups, a level label directly on the PR takes precedence over labels inherited from its linked issue.
