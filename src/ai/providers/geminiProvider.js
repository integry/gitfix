...
/**
 * Google Gemini provider implementation using Google AI API
 */
export class GeminiProvider extends AIProviderInterface {
    ...
    async validateConfiguration() {
        ...
        try {
            ...
            if (!response.ok) {
                logger.error({ status: response.status }, 'Google AI API validation failed');
                return false;
            }

            logger.info('Gemini provider configuration validated successfully');
            return true;
        } catch (error) {
            logger.error({ error: error.message }, 'Failed to validate Gemini configuration');
            return false;
        }
    }
    ...
    async executeCode({ worktreePath, issueRef, githubToken, customPrompt, isRetry = false, retryReason, branchName, modelName }) {
        ...
        logger.info({
            ...
            isRetry,
            retryReason
        }, isRetry ? 'Starting Gemini execution (RETRY)...' : 'Starting Gemini execution...');
...
- The system will automatically commit your changes after you complete the modifications`;
            ...
            }, 'Generated Gemini prompt with safety rules');

            // Create a temporary script to execute the Gemini interaction
...
export GOOGLE_AI_API_KEY="${this.apiKey}"
export PYTHONPATH="/usr/local/lib/python3.11/site-packages"

# Change to worktree directory
cd "${worktreePath}"

# Create Python script for Gemini interaction
cat > /tmp/gemini_executor.py << 'EOF'
import os
import json
import subprocess
import sys
...
def call_gemini_api(messages, model):
    ...
    if system_content and contents and contents[0]["role"] == "user":
        contents[0]["parts"][0]["text"] = system_content + "\\n\\n" + contents[0]["parts"][0]["text"]
...
"""
    ...
    print(f"Starting Gemini execution with model ${model}")
    ...
    while turn < max_turns:
        ...
        try:
            ...
            assistant_message = call_gemini_api(messages, "${model}")
    ...
    with open("/tmp/gemini_execution_log.json", "w") as f:
        json.dump(log_data, f, indent=2)
    
    print("\\n" + "=" * 50)
    print(f"Execution completed. Total turns: {turn}")
    print("Conversation log saved to /tmp/gemini_execution_log.json")
...
python3 /tmp/gemini_executor.py

# Clean up
rm -f /tmp/gemini_executor.py
`;

        const tempScriptPath = path.join(os.tmpdir(), `gemini-script-${issueRef.number}-${Date.now()}.sh`);
    ...
    parseExecutionResult(result, model, executionTime) {
        ...
        try {
            const logPath = '/tmp/gemini_execution_log.json';
            ...
            logger.warn({ error: parseError.message }, 'Failed to parse Gemini execution log');
        }
