...
test('Model Aliases Configuration', async (t) => {
    await t.test('should resolve known aliases to full model IDs', () => {
        assert.strictEqual(resolveModelAlias('opus'), 'claude-opus-4-0');
        assert.strictEqual(resolveModelAlias('sonnet'), 'claude-sonnet-4-0');
        assert.strictEqual(resolveModelAlias('sonnet37'), 'claude-3-7-sonnet-latest');
        assert.strictEqual(resolveModelAlias('sonnet35'), 'claude-3-5-sonnet-latest');
        assert.strictEqual(resolveModelAlias('haiku'), 'claude-3-5-haiku-latest');
    });
    ...
    await t.test('should use default model when no model specified', () => {
        ...
        assert.strictEqual(defaultModel, 'claude-sonnet-4-0');
    ...
    await t.test('getDefaultModel should return sonnet as default', () => {
        assert.strictEqual(getDefaultModel(), 'claude-sonnet-4-0');
