...
export class SetupRequestError extends Error {
  constructor(message = 'Invalid local setup request') { super(message); this.name = 'SetupRequestError'; }
}
...
export const parseDesktopSetupRequest = (input: unknown): DesktopSetupRequest => {
  ...
  if (typeof value.sessionId !== 'string' || !SESSION.test(value.sessionId) || typeof value.reinitialize !== 'boolean') throw new SetupRequestError();
  const root = record(value.root);
  if (root.mode !== 'default' && root.mode !== 'resume') throw new SetupRequestError();
  exact(root, ['mode']);
  if (!Array.isArray(value.agents) || value.agents.length > AGENTS.size
  ...
  if (github.mode === 'keep' || github.mode === 'demo' || github.mode === 'relay') exact(github, ['mode']);
    ...
    if (!bounded(github.appId, 20) || !INTEGER.test(github.appId) || !bounded(github.installationId, 20)
      || !INTEGER.test(github.installationId) || typeof github.privateKeyCapability !== 'string'
      || !CAPABILITY.test(github.privateKeyCapability)) throw new SetupRequestError('Invalid GitHub App configuration');
  } else throw new SetupRequestError('Invalid GitHub configuration');
  ...
  if (value.repository !== null) {
    ...
    if (!bounded(repository.fullName, 140) || extra !== undefined || !owner || !USERNAME.test(owner) || !name
      || !REPOSITORY_NAME.test(name) || name === '.' || name === '..'
