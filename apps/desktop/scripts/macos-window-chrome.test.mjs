...
async function exerciseChrome(context, native) {
  ...
  try {
    ...
    const capture = async (name, title) => {
      ...
      if (process.env.PROPR_DESKTOP_MAC_CHROME_PREVIEWS) {
        ...
        const width = (await sharp(pixels).metadata()).width;
        await sharp(pixels).extract({ left: 0, top: 0, width, height: name === 'entry' ? 150 : 100 }).toFile(join(output, filename));
    ...
    for (const width of [1280, 880]) {
      ...
      // shipped CSS and real browser layout, including the actual logo asset.
...
it('macOS renderer uses the real platform selectors and keeps the logo clear of traffic lights', { timeout: 120_000 }, context => exerciseChrome(context, false));
